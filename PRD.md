...
2. A sheet opens: time entered on scrolling hour, minute and AM/PM wheels (12-hour), label, repeat days, difficulty. Difficulty is pre-filled from `settings.default_difficulty`.
...
- **Time input:** ✅ Three scrolling wheels — hour 1–12, minute 00–59, AM/PM — with the selected value centred and its neighbours dimmed, and the meridiem following the hour wheel across the 11/12 boundary the way a clock face does. Revised twice: first from the original numeric keypad, which was unambiguous but cost four deliberate taps and read as a form field rather than a clock; then from 24-hour to 12-hour at the user's request, which also moved the alarm list and the next-alarm summary onto the 12-hour display so the app reads one clock rather than two. Storage, scheduling and every calculation stay on the 0–23 hour. Only real hours and minutes exist on the wheels, so an invalid time still cannot be entered, and each wheel is an accessibility adjustable so a screen reader never has to perform a drag. Still not a reproduction of the Stitch web control. *(2026-09-11)*
